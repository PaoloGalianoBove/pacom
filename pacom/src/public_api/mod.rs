...
impl PacomRuntime {
	...
	pub async fn invoke_method(&self, logical_method: &str, payload: Vec<u8>) -> Result<Vec<u8>, PacomError> {
	...
	pub async fn publish_event(&self, logical_topic: &str, payload: Vec<u8>) -> Result<(), PacomError> {
	...
	pub async fn subscribe_event<F>(&self, logical_topic: &str, callback: F) -> Result<(), PacomError>
	where
		F: Fn(Vec<u8>) + Send + Sync + 'static,
	{
